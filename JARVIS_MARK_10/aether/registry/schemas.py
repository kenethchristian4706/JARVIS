...
class MoveFileSchema(BaseModel):
    source: str = Field(description="Path of the source file to move.")
    destination: Optional[str] = Field(None, description="Path of the destination folder or file.")

class CopyFileSchema(BaseModel):
    source: str = Field(description="Path of the source file to copy.")
    destination: Optional[str] = Field(None, description="Path of the destination folder or file.")

class RenameFileSchema(BaseModel):
    source: str = Field(description="Current filename or path to rename.")
...
class OpenFileSchema(BaseModel):
    filename: str = Field(description="Name of the file to open.")
...
class ListDirectorySchema(BaseModel):
    path: Optional[str] = Field(None, description="Path of the directory to list.")

class CompressFilesSchema(BaseModel):
    sources: List[str] = Field(description="List of paths of files or folders to compress.")
    output: str = Field(description="Path of the output zip file to create.")

class ExtractArchiveSchema(BaseModel):
    archive: str = Field(description="Path of the zip archive to extract.")
    destination: Optional[str] = Field(None, description="Path of the destination folder to extract into.")

class FileInfoSchema(BaseModel):
    filename: str = Field(description="Name of the file to check information for.")
...
class DownloadFileSchema(BaseModel):
    ...
    destination: Optional[str] = Field(None, description="Path of the destination directory to save the download.")
...
class SetClipboardSchema(BaseModel):
    text: str = Field(description="Text to copy/write to the Windows clipboard.")
