"""
platform/mac/clipboard_tools.py

macOS implementation of ClipboardAPI using pyperclip.
"""
...
class MacClipboardAPI(ClipboardAPI):
    def clear_clipboard(self) -> dict:
        try:
            logger.info("Clearing macOS clipboard.")
            pyperclip.copy("")
            if pyperclip.paste() == "":
                return {
                    "success": True,
    ...
    def get_clipboard(self) -> dict:
        try:
            logger.info("Retrieving macOS clipboard content.")
    ...
    def set_clipboard(self, text: str) -> dict:
        try:
            logger.info("Setting macOS clipboard content.")
            pyperclip.copy(text)
