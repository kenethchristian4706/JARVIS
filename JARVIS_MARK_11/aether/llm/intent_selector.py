...
def select_intents(query: str) -> list[str]:
    """
    Identifies one or more categories of intent matching the user's multi-action query.
    Returns unique intents only.
    """
    from aether.llm.model import generate_completion
    from aether.llm.grammar import repair_and_parse_json

    prompt = f"""You are the Multi-Intent Selector for Aether, a local offline desktop assistant.
Your task is to classify the user's natural language query into one or more of the following operational intents:
1. application_management: Opening, closing, listing, or switching focus between desktop applications.
2. file_system: Moving, copying, renaming, deleting, searching, opening files, creating/deleting folders, compressing, or extracting archives.
3. browser_operations: Web search, YouTube search, opening URLs, downloading files, opening/closing/switching tabs.
4. system_control: System shutdown, restart, sleep, lock, volume adjustment, muting, or screen brightness controls.
...
Do not include any explanations, formatting, markdown code blocks, or preamble. Return ONLY the JSON object.
"""
    ...
    intent_schema = {
        ...
        "properties": {
            "intents": {
                ...
                "items": {
                    "type": "string",
                    "enum": ["application_management", "file_system", "browser_operations", "system_control"]
                }
            }
        },
        "required": ["intents"]
    ...
    logger.info("Running LLM Multi-Intent Selection stage...")
    raw_response = generate_completion(prompt, json_schema=intent_schema, max_tokens=100)
    
    try:
        parsed = repair_and_parse_json(raw_response)
        intents = parsed.get("intents", [])
        # Return unique intents only
        return list(set(intents))
    except Exception as e:
        logger.error(f"Multi-Intent Selection failed: {e}. Raw response: {raw_response}")
        # Fallback to single category selection
        try:
            category, _ = select_intent(query)
            return [category]
        except Exception:
            raise
