...
def resolve_app_name(extracted_name: str) -> str:
    ...
    matches_inst = difflib.get_close_matches(clean_name, installed_names, n=1, cutoff=0.6)
    if matches_inst:
    ...
    matches_alias = difflib.get_close_matches(clean_name, list(APP_ALIASES.keys()), n=1, cutoff=0.6)
    if matches_alias:
