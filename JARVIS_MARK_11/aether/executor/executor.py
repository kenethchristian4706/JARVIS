...
class ToolResultString(str):
    """
    A custom string subclass that holds the raw result dictionary or object from a tool.
    Used to pass structured information back from execute_tool without changing its signature.
    """
    def __new__(cls, value, raw_result=None):
        obj = super().__new__(cls, value)
        obj.raw_result = raw_result
        return obj

# Map tool names to python functions
# ... (rest of TOOL_MAP is same) ...
TOOL_MAP = {
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            if data:
                ...
                output_str = message
            return success, ToolResultString(output_str, raw_result=result)
            
        return True, ToolResultString(str(result), raw_result=result)
