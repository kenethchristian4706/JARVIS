...
def is_multi_action(query: str) -> bool:
    """
    Detects if the query contains multiple actions.
    Checks for conjunctions or commas, then splits the query to ensure there are at least two meaningful actions.
    """
    import re
    query_lower = query.lower()
    
    # 1. Check for conjunctions/commas
    conjunctions = [r"\band\b", r"\bthen\b", r"\bafter\b", ","]
    has_conjunction = any(re.search(conj, query_lower) for conj in conjunctions)
    if not has_conjunction:
        return False
        
    # 2. Split by conjunctions/commas and ensure we have at least two meaningful clauses
    parts = re.split(r"\band\b|\bthen\b|\bafter\b|,", query_lower)
    meaningful_parts = 0
    for part in parts:
        if re.search(r"\w+", part):
            meaningful_parts += 1
            
    return meaningful_parts >= 2
...
def run_query(query: str) -> Dict[str, Any]:
    ...
    try:
        # Step 0: Check if multi-action query
        if is_multi_action(query):
            # 1. select_intents
            from aether.llm.intent_selector import select_intents
            intents = select_intents(query)
            logger.info(f"Detected intents: {intents}")
            
            # 2. collect_candidate_tools
            from aether.planner.tool_collector import collect_candidate_tools
            candidate_tools = collect_candidate_tools(intents)
            logger.info(f"Collected candidate tools: {candidate_tools}")
            
            # 3. generate_plan
            plan_start = time.perf_counter()
            from aether.llm.planner import generate_plan
            plan = generate_plan(query, candidate_tools)
            metrics["intent_time"] = time.perf_counter() - plan_start
            logger.info(f"Generated plan: {plan}")
            
            steps["plan"] = plan.dict()
            
            # 4. validate_plan
            from aether.planner.validator import validate_plan
            val_start = time.perf_counter()
            is_valid, err = validate_plan(plan, candidate_tools)
            ...
            if not is_valid:
                ...
                return {
                    "success": False,
                    "error": f"Validation failure: {err}",
                    "steps": steps,
                    "output": "",
                    "metrics": metrics
                }
            ...
            exec_success, exec_output = execute_plan(plan)
            metrics["execution_time"] = time.perf_counter() - exec_start
            logger.info(f"Executed actions results: success={exec_success}, output={exec_output}")
